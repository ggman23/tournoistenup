...
<head>
  ...
  <style>
    /* ═══════════════════════════════════════════════════════════════════════
       V5 — Premium Light / Stripe aesthetic
       ═══════════════════════════════════════════════════════════════════════ */
    :root {{
      --bg:          #f6f9fc;
      --surface:     #ffffff;
      ...
      --danger:      #c0392b;
      --danger-50:   #fff4f2;
      --shadow-sm:   0 2px 5px 0 rgba(60,66,87,.06), 0 1px 1px 0 rgba(0,0,0,.04);
      --shadow-md:   0 7px 14px 0 rgba(60,66,87,.10), 0 3px 6px 0 rgba(0,0,0,.07);
      --shadow-lg:   0 15px 35px 0 rgba(60,66,87,.13), 0 5px 15px 0 rgba(0,0,0,.07);
      --radius-sm:   8px;
      --radius:      12px;
      --radius-lg:   16px;
      --radius-xl:   24px;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero::before {{
      ...
      background: linear-gradient(90deg, #635bff 0%, #7c75ff 40%, #0073e6 70%, #00c9ff 100%);
    }}
    .hero-inner {{ display: flex; align-items: center; gap: 24px; flex-wrap: wrap; }}
    .hero-title {{
      font-size: 1.55rem; font-weight: 800; margin: 0; color: var(--text);
      letter-spacing: -0.03em;
    }}
    .hero-sub {{ color: var(--text-muted); font-size: .83em; margin-top: 3px; font-weight: 400; }}
    /* ── Stat cards Stripe ──────────────────────────────────────────────────── */
    .stats-row {{ display: flex; gap: 14px; flex-wrap: wrap; margin-left: auto; }}
    .stat-card {{
      background: var(--surface);
      border: 1px solid var(--border);
    ...
    .stat-card .stat-label {{
      font-size: .68em; color: var(--text-muted);
      text-transform: uppercase; letter-spacing: .08em; font-weight: 600;
    }}
    .stat-card .stat-value {{
      font-size: 1.55em; font-weight: 800; color: var(--text);
      line-height: 1; letter-spacing: -0.025em;
    }}
    .stat-card.stat-primary {{ border-top: 3px solid var(--primary); }}
    .stat-card.stat-accent  {{ border-top: 3px solid #ff4c91; }}
    .stat-card.stat-primary .stat-value {{ color: var(--primary); }}
    .stat-card.stat-accent  .stat-value {{ color: #ff4c91; }}
    /* ── Surface cards ────────────────────────────────────────────────────── */
    ...
    /* ── Épreuves ─────────────────────────────────────────────────────────── */
    .ep-line  {{ margin-bottom: 4px; white-space: nowrap; line-height: 1.5; }}
    .ep-nature {{ color: var(--text); font-weight: 600; }}
    .ep-age    {{ color: var(--text-muted); font-size: .85em; }}
    .ep-range  {{ color: var(--primary); font-weight: 500; }}
    .ep-tarif  {{
    ...
    /* ── Badges ──────────────────────────────────────────────────────────── */
    .badge        {{ font-size: .72em; font-weight: 600; letter-spacing: .01em; }}
    .fmt-badge    {{ font-size: .82em; padding: .35em .7em; border-radius: 20px; }}
    .fmt-ep-badge    {{ font-size: .72em; padding: .22em .5em; vertical-align: middle; border-radius: 20px; }}
    .statut-badge    {{ font-size: .7em; padding: .24em .6em; vertical-align: middle; border-radius: 20px; }}
    .srf-badge       {{ font-size: .72em; border-radius: 20px; padding: .2em .55em; }}
    .ep-comment      {{ border-top: 1px dashed var(--border); margin-top: 5px; padding-top: 3px; }}
    /* ── Table ──────────────────────────────────────────────────────────── */
    ...
    table.dataTable thead.table-dark th {{ background: var(--bg) !important; color: var(--text-muted) !important; }}
    table.dataTable tbody tr {{ transition: background .15s ease; }}
    table.dataTable tbody tr:hover {{ background: var(--primary-50) !important; }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd) {{ background: transparent; }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd):hover {{ background: var(--primary-50) !important; }}
    table.dataTable tr.table-warning {{ background: #fffbf0 !important; }}
    table.dataTable tr.table-warning:hover {{ background: #fef3c7 !important; }}
    .dataTables_wrapper .dataTables_paginate .page-link {{
      color: var(--text-muted); border-color: var(--border); border-radius: 8px !important; margin: 0 2px;
    }}
    ...
    #filter-bar label.form-check-label,
    #filter-bar small {{ color: var(--text-muted); }}
    #filter-bar .fw-semibold {{
    ...
    #filter-bar .form-control::placeholder {{ color: var(--text-subtle) !important; }}
    ...
    #filter-bar .border-bottom,
    #filter-bar .border-top {{ border-color: var(--border-2) !important; }}
    ...
    #filter-bar .btn-outline-primary.btn-primary {{
      background: var(--primary) !important;
      border-color: var(--primary) !important;
      color: #fff !important;
      box-shadow: 0 2px 5px rgba(99,91,255,.3);
    }}
    #filter-bar .btn-outline-secondary[onclick="resetFilters()"] {{
      color: var(--danger) !important;
      border-color: var(--border) !important;
      background: var(--surface) !important;
    }}
    #filter-bar .btn-outline-secondary[onclick="resetFilters()"]:hover {{
    ...
    #filter-bar .form-check-input:focus {{ box-shadow: 0 0 0 3px rgba(99,91,255,.15) !important; }}
    #filter-bar .form-check-label {{ color: var(--text) !important; font-size: .85em !important; font-weight: 500; }}
    #filter-bar .form-check-label.fw-semibold {{ color: var(--text-muted) !important; }}
    #filter-bar label[for="chk-hide-vert"]   {{ color: var(--success) !important; font-weight: 600 !important; }}
    #filter-bar label[for="chk-hide-orange"] {{ color: var(--warning) !important; font-weight: 600 !important; }}
    #filter-count {{ color: var(--text-muted) !important; font-weight: 500; }}
    /* ── Chips premium pills ─────────────────────────────────────────────── */
    .dept-group {{ display: flex; align-items: center; flex-wrap: wrap; gap: 5px; font-size: .82em; padding: 3px 0; }}
    .dept-ligue-btn {{
      font-size: .7em; white-space: nowrap; user-select: none;
    ...
    .dept-ligue-btn:hover {{
      background: var(--primary-50) !important; color: var(--primary) !important;
      border-color: var(--primary-100) !important;
    }}
    .dept-chips {{ display: inline-flex; flex-wrap: wrap; gap: 4px; margin-left: 5px; }}
    .dept-chip {{
      display: inline-flex; align-items: center; gap: 2px;
      padding: 3px 10px;
      border: 1.5px solid var(--border); border-radius: 20px;
      cursor: pointer; background: var(--surface); color: var(--text);
      white-space: nowrap; font-size: .92em; font-weight: 500;
      transition: all .15s ease;
    }}
    .dept-chip:hover {{ background: var(--primary-50); border-color: var(--primary-100); color: var(--primary); }}
    .dept-chip:has(input:checked) {{
      background: var(--primary); color: #fff; border-color: var(--primary);
      box-shadow: 0 2px 6px rgba(99,91,255,.28);
    }}
    ...
    .multi-panel {{
      ...
      min-width: 250px; max-width: 460px; max-height: 310px; overflow-y: auto;
      margin-top: 5px;
    }}
    .dropdown-menu {{
      border: 1.5px solid var(--border) !important;
    ...
    .dt-buttons .btn:hover {{ background: var(--bg) !important; border-color: #c0c8d8 !important; }}
    .dt-buttons {{ margin-bottom: 12px; }}
    .fav-btn {{
      background: none; border: none; cursor: pointer;
      font-size: 1.25em; padding: 0 4px; color: var(--text-subtle);
      line-height: 1; transition: all .2s ease;
    }}
    .fav-btn:hover {{ transform: scale(1.2); }}
    .fav-btn.fav-active {{ color: #f59e0b; }}
    /* ── View tabs — floating pill bar ──────────────────────────────────── */
    .view-tabs-wrap {{
      display: inline-flex;
      background: var(--surface);
      border: 1.5px solid var(--border);
    ...
    .view-tab:hover {{ background: var(--surface-3) !important; color: var(--text) !important; }}
    .view-tab.btn-primary {{
      background: var(--primary) !important;
      color: #fff !important;
      box-shadow: 0 2px 8px rgba(99,91,255,.35);
    }}
    .view-tab.btn-warning   {{ background: #f59e0b !important; color: #fff !important; }}
    .view-tab.btn-outline-warning {{ color: #f59e0b !important; }}
    /* ── Calendrier Premium ──────────────────────────────────────────────── */
    .cal-grid {{ display: grid; grid-template-columns: repeat(7, 1fr); gap: 5px; }}
    .cal-dow  {{
      text-align: center; font-weight: 700; font-size: .7em;
      padding: 8px 2px; text-transform: uppercase; letter-spacing: .08em;
      color: var(--text-subtle); background: transparent;
    }}
    .cal-cell {{
      min-height: 88px; border: 1.5px solid var(--border-2); border-radius: var(--radius);
    ...
    .cal-has-events {{ background: #fafbff; border-color: #ddd9ff; }}
    .cal-today  {{ border: 2px solid var(--primary) !important; background: var(--primary-50) !important; }}
    .cal-empty  {{ border: none !important; background: transparent !important; cursor: default; }}
    .cal-day-num {{ font-size: .82em; font-weight: 700; color: var(--text-muted); line-height: 1.2; }}
    .cal-today .cal-day-num {{ color: var(--primary); }}
    .cal-count  {{ font-weight: 800; font-size: 1.1em; color: var(--primary); line-height: 1.3; }}
    .cal-chip   {{
      font-size: .66em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
      padding: 2px 6px; border-radius: 20px; margin-top: 2px; font-weight: 600;
    }}
    .cal-chip-more {{ font-size: .66em; color: var(--text-subtle); margin-top: 2px; font-style: italic; }}
    .cal-panel  {{
      border: 1.5px solid var(--border); border-radius: var(--radius); padding: 18px 20px;
      background: var(--surface); margin-top: 16px; box-shadow: var(--shadow-sm);
    }}
    /* ── Gantt Premium ──────────────────────────────────────────────────── */
    .gantt-row   {{
      display: flex; align-items: center; border-bottom: 1px solid var(--border-2);
      min-height: 30px; transition: background .15s ease;
    }}
    .gantt-row:hover {{ background: var(--primary-50); }}
    .gantt-label {{
      width: 210px; min-width: 210px; font-size: .78em; overflow: hidden;
      text-overflow: ellipsis; white-space: nowrap; padding-right: 12px;
      color: var(--text); text-align: right; font-weight: 500;
    }}
    .gantt-bar   {{
      position: absolute; height: 18px; top: 3px; border-radius: 20px;
      cursor: pointer; opacity: .88; transition: all .18s ease;
    }}
    .gantt-bar:hover {{ opacity: 1; transform: translateY(-1px); box-shadow: var(--shadow-md); }}
    .gantt-today {{
      position: absolute; top: 0; bottom: 0; width: 2px;
      background: var(--primary); opacity: .5; pointer-events: none; z-index: 5;
    }}
    /* ── Carte Premium ──────────────────────────────────────────────────── */
    #view-map-wrap {{ position: relative; }}
    #view-map {{
      height: 640px; border-radius: var(--radius-lg); overflow: hidden;
      border: 1.5px solid var(--border); box-shadow: var(--shadow-md);
    }}
    #view-map-wrap.map-fs {{
      position: fixed !important; top: 0; left: 0; right: 0; bottom: 0;
      z-index: 9999; background: #fff; padding: 0;
    }}
    #view-map-wrap.map-fs #view-map {{ height: 100vh !important; border-radius: 0; border: none; }}
    #map-fs-btn {{
      ...
      padding: 7px 16px; font-size: .82em; font-weight: 600; cursor: pointer;
      color: var(--text); box-shadow: var(--shadow-md); transition: all .15s;
    }}
    #map-fs-btn:hover {{ box-shadow: var(--shadow-lg); }}
    .map-popup-btn {{
      ...
      transition: background .15s; box-shadow: 0 2px 6px rgba(99,91,255,.3);
    }}
    .map-popup-btn:hover {{ background: var(--primary-2) !important; color: #fff !important; }}
    .leaflet-popup-content {{ min-width: 220px; font-family: 'Inter', sans-serif !important; }}
    .leaflet-popup-content-wrapper {{ border-radius: var(--radius) !important; box-shadow: var(--shadow-lg) !important; border: 1.5px solid var(--border) !important; }}
    .map-ep-line {{ font-size: .78em; color: var(--text-muted); white-space: nowrap;
                   overflow: hidden; text-overflow: ellipsis; }}
    /* ── Container ──────────────────────────────────────────────────────── */
    .container-fluid {{ max-width: 1600px; padding: 24px 28px !important; }}
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero::before {{
      ...
      background: radial-gradient(800px 220px at 100% 0%, rgba(99,102,241,0.08), transparent 60%),
                  radial-gradient(600px 200px at 0% 100%, rgba(236,72,153,0.05), transparent 60%);
      pointer-events: none;
    }}
    ...
    .hero-sub {{ color: var(--text-muted); font-size: .82em; margin-top:2px; }}
    /* ── Stat cards ────────────────────────────────────────────────────────── */
    .stats-row {{ display:flex; gap:12px; flex-wrap:wrap; margin-left:auto; }}
    .stat-card {{
      background: var(--surface);
      border: 1px solid var(--border);
      border-radius: var(--radius);
    ...
    .stat-card .stat-label {{ font-size: .7em; color: var(--text-muted); text-transform: uppercase; letter-spacing: .06em; font-weight: 600; }}
    .stat-card .stat-value {{ font-size: 1.35em; font-weight: 700; color: var(--text); line-height:1.1; letter-spacing: -0.02em; }}
    .stat-card.stat-primary::before {{ content:""; position:absolute; left:0; top:0; bottom:0; width:3px; background: linear-gradient(180deg, var(--primary), var(--primary-2)); }}
    .stat-card.stat-accent::before  {{ content:""; position:absolute; left:0; top:0; bottom:0; width:3px; background: var(--accent); }}
    .stat-card.stat-primary .stat-value {{ color: var(--primary); }}
    ...
    /* ── Links & text ─────────────────────────────────────────────────────── */
    .tournament-link {{ font-weight: 600; color: var(--primary); text-decoration: none; transition: color .15s; }}
    .tournament-link:hover {{ color: var(--primary-2); text-decoration: underline; text-underline-offset: 2px; }}
    /* ── Epreuves ─────────────────────────────────────────────────────────── */
    .ep-line  {{ margin-bottom: 3px; white-space: nowrap; line-height: 1.45; }}
    .ep-nature {{ color: var(--text); font-weight: 600; }}
    .ep-age    {{ color: var(--text-muted); font-size: .85em; }}
    .ep-range  {{ color: var(--primary); font-weight: 500; }}
    .ep-tarif  {{ background: var(--primary-50); color: var(--primary); border-radius: 4px; padding: 1px 6px; font-weight: 600; font-size: .82em; }}
    /* ── Badges ──────────────────────────────────────────────────────────── */
    .badge        {{ font-size: .72em; font-weight: 600; letter-spacing: .01em; }}
    .fmt-badge    {{ font-size: .8em; padding: .35em .65em; border-radius: 5px; box-shadow: 0 1px 2px rgba(0,0,0,.08); }}
    .fmt-ep-badge    {{ font-size: .72em; padding: .22em .5em; vertical-align: middle; opacity: .92; border-radius: 4px; }}
    .statut-badge    {{ font-size: .7em; padding: .24em .55em; vertical-align: middle; border-radius: 4px; }}
    .srf-badge       {{ font-size: .72em; border-radius: 4px; padding: .2em .5em; }}
    .ep-comment      {{ border-top: 1px dashed var(--border); margin-top: 5px; padding-top: 3px; }}
    /* ── Table DataTables ─────────────────────────────────────────────────── */
    table.dataTable {{ border-collapse: separate !important; border-spacing: 0 !important; }}
    table.dataTable td {{ vertical-align: middle; border-top: 1px solid var(--border-2) !important; font-size: .88em; }}
    table.dataTable thead th {{
    ...
    table.dataTable.table-dark thead th,
    table.dataTable thead.table-dark th {{ background: var(--surface-2) !important; color: var(--text-muted) !important; }}
    table.dataTable tbody tr {{ transition: background .12s; }}
    table.dataTable tbody tr:hover {{ background: var(--primary-50) !important; }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd) {{ background: transparent; }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd):hover {{ background: var(--primary-50) !important; }}
    table.dataTable tr.table-warning {{ background: #fffbeb !important; }}
    table.dataTable tr.table-warning:hover {{ background: #fef3c7 !important; }}
    .dataTables_wrapper .dataTables_length select,
    .dataTables_wrapper .dataTables_filter input,
    .dataTables_wrapper .dataTables_info,
    .dataTables_wrapper .dataTables_paginate {{ font-size: .85em; }}
    .dataTables_wrapper .dataTables_paginate .page-link {{ color: var(--text-muted); border-color: var(--border); border-radius: 6px !important; margin: 0 2px; }}
    .dataTables_wrapper .dataTables_paginate .active .page-link {{ background: var(--primary); border-color: var(--primary); color:#fff; }}
    /* ── Filter bar — light, professionnel ────────────────────────────────── */
    #filter-bar {{
      background: var(--surface);
      border: 1px solid var(--border);
      border-radius: var(--radius-xl);
    ...
    #filter-bar .border-bottom,
    #filter-bar .border-top {{ border-color: var(--border-2) !important; }}
    /* Boutons dropdown (épreuves/surface/format/statut) */
    #filter-bar .btn-outline-primary,
    #filter-bar .btn-outline-secondary {{
      color: var(--text) !important;
      border: 1px solid var(--border) !important;
    ...
    #filter-bar .form-check-input:focus {{ box-shadow: 0 0 0 3px rgba(99,102,241,.15) !important; }}
    #filter-bar .form-check-label {{ color: var(--text) !important; font-size: .85em !important; font-weight: 500; }}
    #filter-bar .form-check-label.fw-semibold {{ color: var(--text-muted) !important; }}
    #filter-bar label[for="chk-hide-vert"]   {{ color: var(--success) !important; }}
    #filter-bar label[for="chk-hide-orange"] {{ color: var(--warning) !important; }}
    /* filter-count */
    #filter-count {{ color: var(--text-muted) !important; font-weight: 500; }}
    /* Stat card neutre */
    .stat-card.old-inline {{ border-radius: 8px; padding: 8px 16px; color: white; display: inline-block; margin-right: 8px; margin-bottom: 6px; font-weight: 600; }}
    ...
    .fav-btn {{ background: none; border: none; cursor: pointer; font-size: 1.2em;
               padding: 0 4px; color: var(--text-subtle); line-height: 1; transition: all .15s; }}
    .fav-btn:hover {{ transform: scale(1.15); }}
    .fav-btn.fav-active {{ color: var(--warning); }}
    /* Comités (depts) */
    .dept-group {{ display: flex; align-items: center; flex-wrap: wrap; gap: 4px; font-size: .8em; padding: 4px 0; }}
    .dept-ligue-btn {{
      font-size: .7em; white-space: nowrap; user-select: none;
    ...
    .dept-ligue-btn:hover {{ opacity: .85; background: var(--primary-50) !important; color: var(--primary) !important; border-color: var(--primary); }}
    .dept-chips {{ display: inline-flex; flex-wrap: wrap; gap: 3px; margin-left: 4px; }}
    .dept-chip {{
      display: inline-flex; align-items: center; gap: 2px;
      padding: 2px 8px;
      border: 1px solid var(--border); border-radius: 5px;
      cursor: pointer; background: var(--surface); color: var(--text);
      white-space: nowrap; font-size: .96em;
      transition: all .12s ease;
    }}
    .dept-chip:hover {{ background: var(--surface-2); border-color: #cbd5e1; }}
    .dept-chip:has(input:checked) {{
      background: var(--primary); color: #fff; border-color: var(--primary);
    ...
    .multi-panel {{
      ...
      padding: 12px 14px;
      box-shadow: var(--shadow-lg);
      min-width: 240px; max-width: 440px; max-height: 300px; overflow-y: auto;
      margin-top: 4px;
    }}
    .dropdown-menu {{
      border: 1px solid var(--border) !important;
    ...
    .view-tab:hover {{ background: var(--surface-2) !important; color: var(--text) !important; }}
    .view-tab.btn-primary {{
      background: var(--text) !important;
      color: #fff !important;
      box-shadow: 0 1px 2px rgba(15,23,42,.15);
    }}
    .view-tab.btn-warning {{
      background: var(--warning) !important;
      color: #fff !important;
    }}
    .view-tab.btn-outline-warning {{ color: var(--warning) !important; }}
    /* ── Calendrier ──────────────────────────────────────────────────────── */
    .cal-grid {{ display: grid; grid-template-columns: repeat(7, 1fr); gap: 4px; }}
    .cal-dow  {{
      text-align: center; font-weight: 600; font-size: .72em;
      padding: 7px 2px; text-transform: uppercase; letter-spacing: .06em;
      background: var(--surface-2); color: var(--text-muted);
      border-radius: var(--radius-sm);
    }}
    .cal-cell {{
      min-height: 82px; border: 1px solid var(--border-2); border-radius: var(--radius-sm);
      padding: 6px 7px; cursor: pointer; transition: all .15s ease;
      background: var(--surface);
    }}
    .cal-cell:hover:not(.cal-empty) {{ background: var(--primary-50); border-color: var(--primary); transform: translateY(-1px); box-shadow: var(--shadow-sm); }}
    .cal-has-events {{ background: #fafbff; border-color: #dbeafe; }}
    .cal-today  {{ border: 2px solid var(--primary) !important; background: var(--primary-50) !important; }}
    .cal-empty  {{ border: none !important; background: transparent !important; cursor: default; }}
    .cal-day-num {{ font-size: .82em; font-weight: 600; color: var(--text-muted); line-height: 1.2; }}
    .cal-today .cal-day-num {{ color: var(--primary); font-weight: 700; }}
    .cal-count  {{ font-weight: 700; font-size: 1.05em; color: var(--primary); line-height: 1.3; }}
    .cal-chip   {{
      font-size: .67em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
      padding: 2px 5px; border-radius: 3px; margin-top: 2px; font-weight: 500;
    }}
    .cal-chip-more {{ font-size: .67em; color: var(--text-muted); margin-top: 2px; font-style: italic; }}
    .cal-panel  {{
      border: 1px solid var(--border); border-radius: var(--radius); padding: 16px 18px;
      background: var(--surface-2); margin-top: 14px; box-shadow: var(--shadow-sm);
    }}
    /* ── Gantt ────────────────────────────────────────────────────────────── */
    .gantt-row   {{
      display: flex; align-items: center; border-bottom: 1px solid var(--border-2);
      min-height: 28px; transition: background .12s;
    }}
    .gantt-row:hover {{ background: var(--primary-50); }}
    .gantt-label {{
      width: 210px; min-width: 210px; font-size: .78em; overflow: hidden;
    ...
    .gantt-bar   {{
      ...
      cursor: pointer; opacity: .88; transition: all .15s ease;
      box-shadow: 0 1px 2px rgba(0,0,0,.08);
    }}
    .gantt-bar:hover {{ opacity: 1; transform: translateY(-1px); box-shadow: 0 3px 8px rgba(0,0,0,.18); }}
    .gantt-today {{
      position: absolute; top: 0; bottom: 0; width: 2px;
      background: var(--primary); opacity: .65; pointer-events: none; z-index: 5;
    }}
    /* ── Carte ────────────────────────────────────────────────────────────── */
    #view-map-wrap {{ position: relative; }}
    #view-map {{
    ...
    #map-fs-btn {{
      ...
      padding: 6px 12px; font-size: .82em; font-weight: 500; cursor: pointer;
      color: var(--text); box-shadow: var(--shadow-md);
      transition: all .15s;
    }}
    #map-fs-btn:hover {{ background: var(--surface-2); border-color: #cbd5e1; }}
    .map-popup-btn {{
    ...
    .map-ref-marker {{
      ...
      box-shadow: 0 0 0 2px var(--primary), 0 2px 10px rgba(99,102,241,.5);
      display: flex; align-items: center; justify-content: center;
      color: white; font-size: 15px; line-height: 1; font-weight: bold;
    }}
    /* ── Container principal ──────────────────────────────────────────────── */
    .container-fluid {{ max-width: 1600px; padding: 20px 24px !important; }}
    @media (max-width: 992px) {{
      .container-fluid {{ padding: 14px 16px !important; }}
      .hero-title {{ font-size: 1.2rem; }}
      .stat-card {{ min-width: 92px; padding: 8px 12px; }}
    }}
...
<div class="container-fluid py-3">
  ...
  <div class="hero">
    <div class="hero-inner">
      <div>
        ...
        <div class="hero-sub">Mis à jour : {html.escape(fetched_str)}</div>
      </div>
      <div class="stats-row">
        ...
        <div class="stat-card stat-primary" style="opacity:.75">
          <span class="stat-label">Épreuves</span>
          <span class="stat-value">{total_ep}</span>
        </div>
  ...
  <!-- Filter bar v3 -->
  <div id="filter-bar">
    <div style="font-size:.68em;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--text-subtle);margin-bottom:14px;display:flex;align-items:center;gap:8px">
      <span style="display:inline-block;padding:3px 10px;background:var(--primary-50);color:var(--primary);border-radius:20px;font-weight:700;font-size:1em;letter-spacing:.08em">FILTRES</span>
    </div>
  ...
  <!-- Onglets de vue (segmented control) -->
  <div class="d-flex gap-3 mb-3 align-items-center flex-wrap">
    <div class="view-tabs-wrap">
      ...
      <button class="btn btn-sm btn-outline-secondary view-tab" id="tab-vacs"
              onclick="showView('vacs')">🏖️ Vacances</button>
...
function _refreshIsochrones() {{
  ...
  if (_isoLoadingKey === cacheKey) return;
