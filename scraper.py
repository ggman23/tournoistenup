...
class TenupScraper:
    def __init__(self, config: dict, cookies_file: Optional[str] = None):
        ...
        self.session.headers.update({
            ...
            # Accept for regular page GETs (browser-like) — AJAX requests override this via AJAX_HEADERS
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8",
            "Accept-Language": "fr-FR,fr;q=0.9,en;q=0.8",
            "Accept-Encoding": "gzip, deflate, br",
            # X-Requested-With and AJAX Accept are set only for AJAX calls (see AJAX_HEADERS)
            # Setting X-Requested-With here caused TenUp to treat the initial page GET as an AJAX
            # request and redirect it to the homepage instead of serving the search form.
        })
        if cookies_file:
    ...
    def _get_form_tokens(self) -> tuple[str, str, str, str]:
        ...
        while True:
            ...
            # Detect session expiry: queue-it redirect OR TenUp homepage redirect
            # (TenUp now sometimes redirects to its own homepage instead of queue-it.net)
            _redirected_away = (
                "queue-it.net" in resp.url
                or resp.url.rstrip("/") in ("https://tenup.fft.fr", "http://tenup.fft.fr")
                or "/recherche/tournois" not in resp.url
            )
            if _redirected_away:
