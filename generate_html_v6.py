...
<head>
  ...
  <style>
    /* ═══════════════════════════════════════════════════════════════════════
       V6 — Dark Pro / Vercel aesthetic
       Full dark mode, glassmorphism, cyan accents, glow effects
       ═══════════════════════════════════════════════════════════════════════ */
    :root {{
      --bg:          #0a0a0f;
      --bg-2:        #0d0d14;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero::before {{
      ...
      background:
        radial-gradient(600px 300px at 0% 0%, rgba(6,182,212,0.12), transparent 60%),
        radial-gradient(500px 250px at 100% 100%, rgba(139,92,246,0.10), transparent 60%);
      pointer-events: none;
    }}
    ...
    .hero-title {{
      ...
      background: linear-gradient(135deg, #e2e8f0 0%, var(--primary) 60%, var(--accent) 100%);
    ...
    .stat-card:hover {{ border-color: var(--primary); box-shadow: var(--shadow-glow); transform: translateY(-1px); }}
    ...
    .stat-card.stat-primary {{ border-color: rgba(6,182,212,.3); }}
    .stat-card.stat-accent  {{ border-color: rgba(139,92,246,.3); }}
    .stat-card.stat-primary .stat-value {{ color: var(--primary); text-shadow: 0 0 20px var(--primary-glow); }}
    .stat-card.stat-accent  .stat-value {{ color: var(--accent); text-shadow: 0 0 20px var(--accent-glow); }}
    /* ── Surface cards ──────────────────────────────────────────────────── */
    ...
    .tournament-link {{
      ...
      transition: all .15s; text-shadow: 0 0 12px transparent;
    }}
    .tournament-link:hover {{
      color: var(--primary-2); text-decoration: underline; text-underline-offset: 2px;
    ...
    .ep-tarif  {{
      ...
      border-radius: 4px; padding: 1px 6px; font-weight: 600; font-size: .82em;
      border: 1px solid rgba(6,182,212,.2);
    }}
    /* ── Badges ─────────────────────────────────────────────────────────── */
    .badge        {{ font-size: .72em; font-weight: 600; letter-spacing: .01em; }}
    .fmt-badge    {{ font-size: .8em; padding: .35em .65em; border-radius: 5px; }}
    ...
    table.dataTable td {{
      vertical-align: middle; border-top: 1px solid var(--border-2) !important;
      font-size: .88em; color: var(--text); background: transparent;
    }}
    table.dataTable thead th {{
      background: var(--surface-2) !important;
      color: var(--text-muted) !important;
      font-weight: 600 !important; font-size: .72em !important;
    ...
    table.dataTable tbody tr:hover {{ background: var(--primary-dim) !important; }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd) {{ background: rgba(255,255,255,.015); }}
    table.dataTable.table-striped tbody tr:nth-of-type(odd):hover {{ background: var(--primary-dim) !important; }}
    table.dataTable tr.table-warning {{ background: rgba(245,158,11,.08) !important; }}
    table.dataTable tr.table-warning:hover {{ background: rgba(245,158,11,.15) !important; }}
    .dataTables_wrapper .dataTables_length select,
    .dataTables_wrapper .dataTables_filter input,
    .dataTables_wrapper .dataTables_info {{ color: var(--text-muted) !important; font-size: .85em; }}
    .dataTables_wrapper .dataTables_filter input,
    .dataTables_wrapper .dataTables_length select {{
    ...
    .dataTables_wrapper .dataTables_paginate .page-link {{
      ...
      color: var(--text-muted) !important; border-radius: 6px !important; margin: 0 2px;
    }}
    .dataTables_wrapper .dataTables_paginate .active .page-link {{
      background: var(--primary) !important; border-color: var(--primary) !important;
      color: #000 !important; box-shadow: 0 0 12px var(--primary-glow);
    }}
    ...
    #filter-bar .fw-semibold {{
      color: var(--text-muted) !important;
      font-size: .68em !important; letter-spacing: .07em;
    ...
    #filter-bar .form-select {{
      ...
      font-size: .88em !important; padding: .4rem .7rem !important;
    ...
    #filter-bar .form-select:focus {{
      ...
      box-shadow: 0 0 0 3px var(--primary-glow) !important; outline: none !important;
      background: var(--glass-2) !important;
    }}
    #filter-bar .form-select option {{ background: var(--surface-3); color: var(--text); }}
    #filter-bar .border-bottom,
    #filter-bar .border-top {{ border-color: var(--border) !important; }}
    ...
    #filter-bar .btn-outline-secondary {{
      ...
      background: var(--glass) !important;
      border-radius: var(--radius-sm) !important;
      font-size: .85em !important; font-weight: 500 !important;
      padding: .35rem .75rem !important; transition: all .15s ease;
    }}
    #filter-bar .btn-outline-primary:hover,
    #filter-bar .btn-outline-secondary:hover {{
      background: var(--glass-2) !important; border-color: rgba(255,255,255,.2) !important;
    }}
    #filter-bar .btn-outline-primary.btn-primary {{
      background: var(--primary) !important; border-color: var(--primary) !important;
      color: #000 !important; box-shadow: 0 0 14px var(--primary-glow);
    }}
    #filter-bar .btn-outline-secondary[onclick="resetFilters()"] {{
      border-color: rgba(239,68,68,.3) !important;
      color: #f87171 !important; background: rgba(239,68,68,.06) !important;
    }}
    #filter-bar .btn-outline-secondary[onclick="resetFilters()"]:hover {{
      background: rgba(239,68,68,.14) !important; border-color: rgba(239,68,68,.5) !important;
    }}
    ...
    #filter-bar .form-check-input:focus {{ box-shadow: 0 0 0 3px var(--primary-glow) !important; }}
    ...
    .dt-buttons .btn {{
      border-radius: var(--radius-sm) !important; font-size: .82em !important;
      font-weight: 500 !important; padding: .35rem .7rem !important; transition: all .15s;
      background: var(--surface-2) !important; border: 1px solid var(--border) !important;
      color: var(--text-muted) !important;
    }}
    .dt-buttons .btn:hover {{ background: var(--surface-3) !important; color: var(--text) !important; }}
    .fav-btn {{
    ...
    .fav-btn.fav-active {{ color: var(--warning); filter: drop-shadow(0 0 6px rgba(245,158,11,.5)); }}
    /* ── Comités dark ───────────────────────────────────────────────────── */
    ...
    .dept-ligue-btn:hover {{ background: var(--primary-dim) !important; color: var(--primary) !important; border-color: rgba(6,182,212,.3) !important; }}
    ...
    .dept-chip {{
      ...
      white-space: nowrap; font-size: .96em; transition: all .12s ease;
    }}
    .dept-chip:hover {{ background: var(--glass-2); border-color: rgba(255,255,255,.15); }}
    ...
    .dropdown-menu .btn-link {{ color: #f87171 !important; }}
    /* ── View tabs : dark segmented ─────────────────────────────────────── */
    .view-tabs-wrap {{
      display: inline-flex;
      background: var(--surface);
      border: 1px solid var(--border);
    ...
    .view-tab {{
      ...
      color: var(--text-muted) !important;
      border-radius: var(--radius-sm) !important;
      font-size: .84em !important; font-weight: 500 !important;
      padding: .4rem .9rem !important;
    }}
    .view-tab:hover {{ background: var(--glass-2) !important; color: var(--text) !important; }}
    .view-tab.btn-primary {{
      background: var(--primary) !important; color: #000 !important;
      box-shadow: 0 0 14px var(--primary-glow);
    }}
    .view-tab.btn-warning   {{ background: var(--warning) !important; color: #000 !important; }}
    ...
    .cal-dow  {{
      ...
      background: var(--surface-3); color: var(--text-muted); border-radius: var(--radius-sm);
    }}
    .cal-cell {{
      min-height: 82px; border: 1px solid var(--border); border-radius: var(--radius-sm);
      padding: 6px 7px; cursor: pointer; transition: all .15s ease; background: var(--surface-2);
    }}
    .cal-cell:hover:not(.cal-empty) {{
      background: var(--primary-dim); border-color: rgba(6,182,212,.4);
      transform: translateY(-1px); box-shadow: 0 0 12px var(--primary-glow);
    }}
    .cal-has-events {{ background: var(--surface-3); border-color: rgba(6,182,212,.2); }}
    .cal-today  {{ border: 2px solid var(--primary) !important; background: var(--primary-dim) !important; box-shadow: 0 0 16px var(--primary-glow); }}
    ...
    .cal-today .cal-day-num {{ color: var(--primary); font-weight: 700; text-shadow: 0 0 10px var(--primary-glow); }}
    ...
    .cal-chip-more {{ font-size: .67em; color: var(--text-subtle); margin-top: 2px; font-style: italic; }}
    .cal-panel  {{
      border: 1px solid var(--border); border-radius: var(--radius); padding: 16px 18px;
      background: var(--surface-2); margin-top: 14px; box-shadow: var(--shadow-md);
    }}
    /* ── Gantt dark ─────────────────────────────────────────────────────── */
    .gantt-row   {{
    ...
    .gantt-row:hover {{ background: var(--primary-dim); }}
    .gantt-label {{
      width: 210px; min-width: 210px; font-size: .78em; overflow: hidden;
      text-overflow: ellipsis; white-space: nowrap; padding-right: 10px;
      color: var(--text-muted); text-align: right; font-weight: 500;
    }}
    .gantt-bar   {{
      position: absolute; height: 18px; top: 2px; border-radius: 4px;
      cursor: pointer; opacity: .85; transition: all .15s ease;
    }}
    .gantt-bar:hover {{ opacity: 1; transform: translateY(-1px); box-shadow: 0 0 14px rgba(6,182,212,.4); }}
    .gantt-today {{
      position: absolute; top: 0; bottom: 0; width: 2px;
      background: var(--primary); opacity: .7; pointer-events: none; z-index: 5;
    ...
    #view-map-wrap.map-fs {{
      ...
      z-index: 9999; background: var(--bg); padding: 0;
    }}
    ...
    #map-fs-btn {{
      ...
      background: rgba(17,17,24,.9); border: 1px solid var(--border); border-radius: var(--radius-sm);
      padding: 6px 12px; font-size: .82em; font-weight: 500; cursor: pointer;
      color: var(--text); box-shadow: var(--shadow-md); backdrop-filter: blur(8px);
      transition: all .15s;
    }}
    #map-fs-btn:hover {{ border-color: var(--primary); box-shadow: 0 0 10px var(--primary-glow); }}
    .map-popup-btn {{
      ...
      box-shadow: 0 0 14px var(--primary-glow); transition: background .15s;
    }}
    .map-popup-btn:hover {{ background: var(--primary-2) !important; color: #000 !important; }}
    ...
    .map-ref-marker {{
      ...
      background: var(--primary); border: 3px solid var(--bg); border-radius: 50%;
      box-shadow: 0 0 0 2px var(--primary), 0 0 16px var(--primary-glow);
      display: flex; align-items: center; justify-content: center;
      color: #000; font-size: 15px; line-height: 1; font-weight: bold;
    }}
    /* ── Container ──────────────────────────────────────────────────────── */
...
<div class="container-fluid py-3">
  ...
  <div class="hero">
    <div class="hero-inner">
      ...
      <div class="stats-row">
        ...
        {('<div class="stat-card" style="border-color:rgba(142,68,173,.35)"><span class="stat-label">Prochain classement</span><span class="stat-value" style="color:#a855f7;text-shadow:0 0 20px rgba(168,85,247,.4)">' + _next_class_str + '</span><span class="stat-label">' + str(_days_to_class) + ' jours</span></div>') if _days_to_class is not None else ''}
      </div>
    </div>
  </div>
  ...
  <!-- Filter bar v3 -->
  <div id="filter-bar">
    <div style="font-size:.68em;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--text-subtle);margin-bottom:12px;display:flex;align-items:center;gap:8px">
      <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--primary);box-shadow:0 0 8px var(--primary-glow)"></span>
      <span style="color:var(--primary);text-shadow:0 0 12px var(--primary-glow)">FILTRES</span>
    </div>
