      ...
      <table id="dt-elite-tout" class="table table-striped table-hover table-sm" style="width:100%">
        <thead>
          ...
          <tr class="elite-filters">
            ...
            <th><input class="form-control form-control-sm" placeholder="2015;2017 ou 2012-2016" title="Séparez par ; pour plusieurs valeurs, ou utilisez - pour une plage (ex: 2012-2016)"></th>
            <th><input class="form-control form-control-sm" placeholder="9;11 ou 8-12" title="Séparez par ; pour plusieurs valeurs, ou utilisez - pour une plage (ex: 8-12)"></th>
...
<head>
  ...
  <style>
    body {{ font-size: 0.875rem; background:#f0f2f7; }}
    ...
    /* ── Filter bar v3 : dark panel ──────────────────────────────────────── */
    #filter-bar {{
      background: linear-gradient(160deg, #1a2540 0%, #1e2d4f 100%);
      border-radius: 14px;
      padding: 16px 20px;
      margin-bottom: 16px;
    ...
    /* Textes dans le filtre */
    #filter-bar .form-label,
    #filter-bar label.form-check-label,
    #filter-bar small {{ color: rgba(255,255,255,.55) !important; }}
    #filter-bar .fw-semibold {{ color: rgba(255,255,255,.7) !important; font-size:.7em; letter-spacing:.04em; text-transform:uppercase; }}
    /* Inputs & selects */
    ...
    #filter-bar .form-control::placeholder {{ color: rgba(255,255,255,.3) !important; }}
    #filter-bar .form-control:focus,
    #filter-bar .form-select:focus {{
      border-color: #4d8eff !important;
      box-shadow: 0 0 0 3px rgba(77,142,255,.2) !important;
      background: rgba(255,255,255,.1) !important;
    }}
    #filter-bar .form-select option {{ background:#1e2d4f; color:white; }}
    /* Border-bottom séparateurs de ligne */
    #filter-bar .border-bottom {{ border-color: rgba(255,255,255,.1) !important; }}
    #filter-bar .border-top    {{ border-color: rgba(255,255,255,.1) !important; }}
    ...
    .multi-panel {{ position:absolute; z-index:200; background:white; border:1px solid #dee2e6; border-radius:10px; padding:8px 10px; box-shadow:0 6px 20px rgba(0,0,0,.18); min-width:220px; max-width:420px; max-height:280px; overflow-y:auto; }}
    /* ── Vue onglets ──────────────────────────────────────────────────────── */
...
<div class="container-fluid py-3">
  ...
  <!-- Filter bar v3 -->
  <div id="filter-bar">
    <div style="font-size:.65em;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.35);margin-bottom:10px">🔽 Filtres</div>
    ...
    <div class="row g-2 align-items-end mt-2 pt-2 border-top">
      ...
      <div class="col-auto">
        ...
        <select class="form-select form-select-sm" id="filter-rang-bas" style="width:100px" onchange="applyFilters()">
          ...
          <option>3/6</option><option>2/6</option><option>1/6</option>
  ...
  <div id="view-coeff" style="display:none" class="bg-white rounded shadow-sm p-3">
    ...
    <div class="table-responsive">
    <table class="table table-bordered table-sm text-center" style="font-size:.82em;width:auto">
      <thead>
...
function planClickDay(ds) {{
  ...
  document.getElementById('planModalTitle').textContent = 'Absence — ' + lbl;
  document.getElementById('planModalInput').value = current;
  document.getElementById('planModalDs').value = ds;
  var modal = new bootstrap.Modal(document.getElementById('planModal'));
  modal.show();
  setTimeout(function() {{ document.getElementById('planModalInput').focus(); document.getElementById('planModalInput').select(); }}, 300);
}}

function planModalConfirm() {{
  var ds      = document.getElementById('planModalDs').value;
  var note    = document.getElementById('planModalInput').value.trim();
  var planning = getPlanningData();
...
function planClearAll() {{
  var modal = new bootstrap.Modal(document.getElementById('planClearModal'));
  modal.show();
}}

function planClearConfirm() {{
  savePlanningData({{}});
  bootstrap.Modal.getInstance(document.getElementById('planClearModal')).hide();
  renderPlanning();
  if (currentView === 'rates') renderRates();
  applyFilters();
}}
...
// Multi-value / range filter helper: parses "2015;2017" or "2012-2016" or "2012-2016;2018"
// Returns a function(strVal)→bool, or null if empty.
function _mkRangeTest(val) {{
  val = (val || '').trim();
  if (!val) return null;
  var parts = val.split(';');
  var tests = [];
  for (var i = 0; i < parts.length; i++) {{
    ...
    if (m) {{
      ...
      (function(ex) {{ tests.push(function(v) {{ return v === ex; }}); }})(p);
    }}
  }}
  if (!tests.length) return null;
  return function(v) {{
    for (var j = 0; j < tests.length; j++) {{ if (tests[j](v)) return true; }}
    return false;
  }};
}}
...
function renderAdvStats() {{
  ...
  var wins  = _ADV.filter(function(r){{ return r[8]==='Oui'; }}).length;
  var losses = total - wins;
  var pct   = total > 0 ? Math.round(wins/total*100) : 0;

  // group by classement le jour du match (index 7), keep rank (index 14)
  var byClass = {{}};
  _ADV.forEach(function(r) {{
    var c = r[7] || '?';
    var rank = typeof r[14]==='number' ? r[14] : 99;
    if (!byClass[c]) byClass[c] = {{wins:0, total:0, rank:rank}};
    byClass[c].total++;
    if (r[8]==='Oui') byClass[c].wins++;
  }});
  ...
  }}).sort(function(a,b){{ return (b[15]-b[14]) - (a[15]-a[14]); }}).slice(0,5);

  // ── Bilan global ──
  var html = '<div class="row g-3 mb-3">';
  html += '<div class="col-auto"><div class="card border-0 bg-light px-3 py-2 text-center"><div class="fs-4 fw-bold">' + total + '</div><div class="small text-muted">Matchs</div></div></div>';
  html += '<div class="col-auto"><div class="card border-0 px-3 py-2 text-center" style="background:rgba(40,167,69,.12)"><div class="fs-4 fw-bold text-success">' + wins + '</div><div class="small text-muted">Victoires</div></div></div>';
  html += '<div class="col-auto"><div class="card border-0 px-3 py-2 text-center" style="background:rgba(220,53,69,.10)"><div class="fs-4 fw-bold text-danger">' + losses + '</div><div class="small text-muted">Défaites</div></div></div>';
  html += '<div class="col-auto"><div class="card border-0 px-3 py-2 text-center" style="background:rgba(13,110,253,.08)"><div class="fs-4 fw-bold text-primary">' + pct + '%</div><div class="small text-muted">Taux victoire</div></div></div>';
  html += '</div>';

  // ── Taux par classement adverse ──
  html += '<div class="row g-3 align-items-start">';
  html += '<div class="col-auto"><h6 class="mb-2 text-muted">Taux de victoire par classement adverse</h6>';
  ...
  // ── Victoires notables ──
  if (notable.length > 0) {{
    html += '<div class="col-auto ms-4"><h6 class="mb-2 text-muted">Victoires notables <small class="text-muted fw-normal">(adversaires depuis bien progressés)</small></h6>';
    html += '<table class="table table-sm table-bordered" style="width:auto;font-size:.85em"><thead><tr style="background:#f8f9fa">';
    html += '<th>Joueur</th><th class="text-center">Class. J</th><th class="text-center">Classement actuel</th><th class="text-center">Best</th><th>Date</th></tr></thead><tbody>';
    notable.forEach(function(r) {{
      html += '<tr><td><a href="https://tenup.fft.fr/palmares/' + r[4] + '" target="_blank" rel="noopener" style="text-decoration:none">' + r[2] + ' <strong>' + r[3] + '</strong></a></td>';
      html += '<td class="text-center">' + r[7] + '</td>';
      html += '<td class="text-center fw-bold text-success">' + r[10] + '</td>';
      html += '<td class="text-center">' + r[11] + '</td>';
      html += '<td>' + r[1] + '</td></tr>';
    }});
    html += '</tbody></table></div>';
  }}
  ...
  _ADV.forEach(function(r) {{
    var key = r[4]; // idCrm
    if (!byPlayer[key]) byPlayer[key] = {{prenom:r[2], nom:r[3], id:r[4], wins:0, total:0, bestClass:'', bestRank:-1}};
    byPlayer[key].total++;
    if (r[8]==='Oui') byPlayer[key].wins++;
    var rb = typeof r[16]==='number' ? r[16] : -1;
    if (rb > byPlayer[key].bestRank) {{ byPlayer[key].bestRank = rb; byPlayer[key].bestClass = r[11]; }}
  }});
  var multi = Object.values(byPlayer).filter(function(p){{ return p.total > 1; }});
  multi.sort(function(a,b){{ return b.total - a.total || b.wins - a.wins; }});
  if (multi.length > 0) {{
    html += '<div class="row g-3 mt-1"><div class="col-auto"><h6 class="mb-2 text-muted">Adversaires rencontrés plusieurs fois</h6>';
    html += '<table class="table table-sm table-bordered" style="width:auto;font-size:.85em"><thead><tr style="background:#f8f9fa">';
    html += '<th>Joueur</th><th class="text-center">Matchs</th><th class="text-center">V</th><th class="text-center">D</th><th class="text-center">%</th><th class="text-center">Meilleur Class.</th></tr></thead><tbody>';
    multi.forEach(function(p) {{
      var pct = Math.round(p.wins/p.total*100);
      var bar = pct>=50 ? 'success' : 'danger';
      html += '<tr><td><a href="https://tenup.fft.fr/palmares/' + p.id + '" target="_blank" rel="noopener" style="text-decoration:none">' + p.prenom + ' <strong>' + p.nom + '</strong></a></td>';
      html += '<td class="text-center fw-bold">' + p.total + '</td>';
      html += '<td class="text-center text-success fw-bold">' + p.wins + '</td>';
      html += '<td class="text-center text-danger fw-bold">' + (p.total-p.wins) + '</td>';
      html += '<td class="text-center"><span class="badge bg-' + bar + '">' + pct + '%</span></td>';
      html += '<td class="text-center">' + (p.bestClass||'?') + '</td></tr>';
    }});
    html += '</tbody></table></div></div>';
  }}

  document.getElementById('adv-stats').innerHTML = html;
}}
...
function _extractCmpData(tr) {{
  ...
  var fmts = [];
  $tr.find('.fmt-badge').each(function() {{ fmts.push($(this).attr('data-fmt') || $(this).text().trim().replace(/^F/,'')); }});
  var epreuves = [];
  $tr.find('.ep-line').each(function() {{
    var $ep = $(this);
...
function _buildCompareHTML() {{
  ...
  h += mkRow('Format(s)', tours.map(function(t) {{ return t.fmts.map(function(f) {{ return '<span class="badge me-1" style="background:' + (_FMT_COLORS[f]||'#6c757d') + ';font-size:.75em">F' + f + '</span>'; }}).join('') || '—'; }}));
...
<div class="modal fade" id="planModal" tabindex="-1" aria-labelledby="planModalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="planModalTitle">Absence</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
      </div>
      <div class="modal-body">
        <label for="planModalInput" class="form-label">Note (laisser vide pour supprimer l'absence) :</label>
        <input type="text" class="form-control" id="planModalInput" placeholder="ex : Vacances, Mariage…"
               onkeydown="if(event.key==='Enter')planModalConfirm();">
        <input type="hidden" id="planModalDs">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
        <button type="button" class="btn btn-primary" onclick="planModalConfirm();">OK</button>
      </div>
    </div>
  </div>
</div>

<!-- Modal : confirmation effacement Planning -->
<div class="modal fade" id="planClearModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Effacer les absences</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
      </div>
      <div class="modal-body">Effacer toutes les absences du Planning ?</div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
        <button type="button" class="btn btn-danger" onclick="planClearConfirm();">Effacer</button>
      </div>
    </div>
  </div>
</div>
